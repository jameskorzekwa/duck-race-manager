...
// A withdrawal whose client never came back. The command row, the status, and
// the revision are exactly what the committed batch writes; what is missing is
// only the reconciliation that would normally follow it in the same request.
...
test("the public race board publishes a skipped heat and an uncontested winner", async (context) => {
  const { database, env, participants } = await setup(context);
  const [first, second, sole] = heatThree(participants);
  assert.equal((await withdraw(env, database, first.registrationId)).status, 201);
