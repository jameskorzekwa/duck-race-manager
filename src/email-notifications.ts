...
const notificationClaimRow = (env: Env, notificationId: string): Promise<NotificationClaimRow | null> =>
  env.DB.prepare(
    `SELECT status, sending_started_at, retry_after
...
export const processEmailNotification = async (
  ...
  if (!claimableStatuses.has(claimRow.status)) return "NOOP";
  ...
  if (
    claimRow.status === "SENDING" && claimRow.sending_started_at !== null
  ...
  if (row === null) {
    await cancelNotification(env, notificationId, "RACE_ASSIGNMENT_CHANGED");
    return "CANCELLED";
  }
