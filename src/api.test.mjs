...
test("creates registration, race-entry, command, and audit records atomically", async (context) => {
  ...
  assert.deepEqual(db.batches[0][6].args.slice(4, 6), ["EMAIL", `REGISTRATION:${db.batches[0][0].args[0]}`]);
  assert.match(db.batches[0][7].sql, /INSERT INTO email_notifications/);
  assert.deepEqual(db.batches[0][7].args.slice(4, 6), ["SMS", `REGISTRATION:${db.batches[0][0].args[0]}`]);
  assert.match(response.headers.get("set-cookie") ?? "", /__Host-quickducks_browser=/);
  assert.match(response.headers.get("set-cookie") ?? "", /HttpOnly/);
