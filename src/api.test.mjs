...
test("creates registration, race-entry, command, and audit records atomically", async (context) => {
  ...
  assert.match(db.batches[0][3].sql, /INSERT INTO participant_notifications/);
  assert.match(db.batches[0][3].sql, /'EMAIL'.*'REGISTRATION_CONFIRMED'/s);
  assert.match(db.batches[0][4].sql, /INSERT INTO participant_notifications/);
  assert.match(db.batches[0][4].sql, /'SMS'.*'REGISTRATION_CONFIRMED'/s);
  assert.doesNotMatch(
    JSON.stringify([db.batches[0][3].args, db.batches[0][4].args]),
    /daisy@example\.com|817|private|lookup/i,
  );
  assert.match(db.batches[0][5].sql, /INSERT INTO audit_events/);
  assert.equal(db.batches[0][5].args.at(-1), JSON.stringify({ created_via: "PUBLIC" }));
