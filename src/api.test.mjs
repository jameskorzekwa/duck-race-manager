...
test("creates registration, race-entry, command, and audit records atomically", async (context) => {
  ...
  assert.match(db.batches[0][3].sql, /INSERT INTO email_notifications/);
  assert.equal(db.batches[0][3].args.includes("EMAIL"), true);
  assert.match(db.batches[0][4].sql, /INSERT INTO email_notifications/);
  assert.equal(db.batches[0][4].args.includes("SMS"), true);
