...
.console-nav a { display:inline-flex; flex:none; min-height:2.75rem; align-items:center; padding:.55rem .7rem; border-radius:.55rem; font-size:.85rem; font-weight:900; text-decoration:none; }
...
.intake-camera { border:4px solid var(--water-dark); background:#eaf7fa; }
.intake-camera-video { display:block; width:100%; max-width:32rem; min-width:0; margin:0 auto; aspect-ratio:4/3; border:3px solid var(--ink); border-radius:.8rem; background:var(--ink); object-fit:cover; }
.intake-camera .actions { margin-top:var(--space-md); }
.intake-camera .button { flex:1 1 11rem; min-height:4rem; }
...
@media (max-width:43.99rem) {
  ...
  .nav a { flex:1 1 0; min-height:2.75rem; padding:.6rem .35rem; text-align:center; }
  ...
  .page-panel { margin:var(--space-sm) auto var(--space-lg); padding:var(--space-md); border-radius:1rem; box-shadow:4px 4px 0 var(--ink); }
  .staff-panel,.heat-bag,.work-area,.pairing-confirmation,.emergency-replacement { min-width:0; max-width:100%; }
  .staff-panel > *,.heat-bag > *,.work-area > *,.pairing-confirmation > * { min-width:0; max-width:100%; }
  .status-section { margin:var(--space-lg) 0; padding:var(--space-md); }
  .cards { gap:var(--space-sm); margin:var(--space-lg) 0; }
  .card,.console-section,.operation-card { padding:var(--space-md); }
  .console-section { margin:var(--space-lg) 0; }
  .notice,.privacy,.emergency-warning { padding:var(--space-sm); overflow-wrap:anywhere; }
  ...
  .section-tools > .button { margin-inline:auto; }
  .console-nav { flex-wrap:wrap; justify-content:center; overflow-x:visible; }
  .button.small { min-height:2.75rem; }
  .app-confirmation { width:calc(100% - 1rem); max-height:calc(100vh - 1rem); padding:var(--space-md); border-radius:1rem; box-shadow:4px 4px 0 var(--ink); }
  .app-confirmation-actions { display:grid; grid-template-columns:repeat(auto-fit,minmax(min(100%,9rem),1fr)); justify-content:center; }
  .app-confirmation-actions .button { width:100%; }
  ...
  .staff-bar { align-items:flex-start; }
  .site-foot { padding:var(--space-lg) 0; }
    ...
    <section class="page-panel">
      ...
      <form method="post" action="/api/v1/registrations" data-registration-form data-protection-ready="${turnstileSiteKey === undefined && !localPreviewWithoutProtection ? "false" : "true"}">
        ...
        <div class="field-grid">
          ...
          ${smsNotificationsEnabled ? '<label data-registration-sms>Phone (optional)<input name="phone" type="tel" autocomplete="tel" inputmode="tel" maxlength="32" placeholder="(817) 320-6150"><span class="field-error" data-field-error="phone" data-contact-error="phone"></span></label>' : ""}
        </div>
        ...
        ${smsNotificationsEnabled ? '<label class="check" data-registration-sms><input name="sms_notifications_enabled" type="checkbox" disabled><span>Send operational race updates by SMS<span class="field-error" data-field-error="sms_notifications_enabled" data-contact-error="sms_notifications_enabled"></span></span></label>' : ""}
        ...
        <div class="console-grid wide">
          <div><details class="operation-card" data-walkup-card><summary>Add walk-up participant</summary>
            ...
            <form data-walkup-form>
              ...
              <div class="field-grid" data-sms-field-slot="walkup"><label>Email<input name="email" type="email" aria-label="Email" maxlength="254"><span class="field-error" data-contact-error="email"></span></label></div>
               <label class="check"><input name="emailNotificationsEnabled" type="checkbox" disabled><span>Send operational race updates by email<span class="field-error" data-contact-error="emailNotificationsEnabled"></span></span></label>
                <div data-sms-consent-slot="walkup"></div>
          ...
          <article class="operation-card" tabindex="-1" data-participant-detail hidden>
            ...
            <form data-participant-edit-form>
              <div class="field-grid"><label>First name<input name="firstName" maxlength="80" required></label><label>Last name<input name="lastName" maxlength="80" required></label></div>
              <div class="field-grid" data-sms-field-slot="edit"><label>Email<input name="email" type="email" aria-label="Email" maxlength="254"><span class="field-error" data-contact-error="email"></span></label></div>
               <label class="check"><input name="emailNotificationsEnabled" type="checkbox" disabled><span>Send operational race updates by email<span class="field-error" data-contact-error="emailNotificationsEnabled"></span></span></label>
                <div data-sms-consent-slot="edit"></div>
              <label>Staff notes<textarea name="notes" maxlength="2000"></textarea></label>
        ...
        <div class="event-detail" role="region" aria-labelledby="event-detail-title" data-event-detail hidden>
          ...
          <div class="console-grid">
            ${isSystemAdmin ? `<details class="operation-card" data-event-config-card hidden><summary>Configure draft</summary>
             ...
             ${isSystemAdmin ? `<article class="operation-card" data-event-sms-card hidden><h3>Participant SMS</h3><p class="muted">Off by default. Enable only for an event that needs text updates; disabling stops pending sends and usage charges.</p><form data-event-sms-form><label class="check"><input name="enabled" type="checkbox"><span>Enable SMS updates for this event</span></label><button class="button secondary" type="submit">Save SMS setting</button></form></article>` : ""}
    ...
    <section class="page-panel operations-panel staff-panel" data-staff-inventory data-live-staff data-system-admin="${isSystemAdmin ? "true" : "false"}" data-roles="${escapeHtml(roles.join(","))}" data-app-origin="${escapeHtml(appOrigin)}">
      ...
      <section class="console-section" id="scan" aria-labelledby="scan-title" data-intake-station>
        ...
        <div data-intake-controls hidden>
          ...
          <article class="operation-card intake-camera" data-intake-photo hidden aria-labelledby="intake-photo-title">
            <p class="eyebrow">Required duck photo</p>
            <h3 id="intake-photo-title" data-intake-photo-prompt>Photograph the duck just added.</h3>
            <p class="muted">Keep this duck at the station. The next intake stays blocked until QuickDucks confirms its photo is stored.</p>
            <video class="intake-camera-video" data-intake-camera muted playsinline></video>
            <canvas data-intake-photo-canvas hidden></canvas>
            <div class="actions">
              <button class="button station-control" type="button" data-capture-intake-photo>Capture and save photo</button>
              <button class="button secondary station-control" type="button" data-retry-intake-photo hidden>Retry required photo</button>
              <button class="button secondary station-control" type="button" data-stop-intake-camera>Stop camera</button>
            </div>
          </article>
