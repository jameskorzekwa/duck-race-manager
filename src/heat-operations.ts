...
interface NotificationHeatRecipient {
  registration_id: string;
  duck_assignment_id: string;
}

const notificationHeatRecipients = (
  env: Env,
  eventId: string,
  heatId: string,
): Promise<D1Result<NotificationHeatRecipient>> => env.DB.prepare(
  `SELECT r.id AS registration_id, da.id AS duck_assignment_id
    ...
    ORDER BY he.slot_number`,
).bind(eventId, heatId).all<NotificationHeatRecipient>();
...
const finalizeResultSet = async (
  ...
  let finalHeatId: string | null = null;
  if (context.round === "ROUND_ONE") {
    ...
    finalHeatId = finalHeat?.id ?? crypto.randomUUID();
  }

  const nextHeat = context.round === "ROUND_ONE"
    ? await env.DB.prepare(
      `SELECT id FROM heats
        WHERE event_id = ? AND round = 'ROUND_ONE' AND heat_number > ?
          AND status NOT IN ('FINALIZED', 'CANCELLED')
          AND ${eligibleEntryCountSql("heats.event_id", "heats.id")} >= 2
        ORDER BY heat_number LIMIT 1`,
    ).bind(eventId, context.heat_number).first<{ id: string }>()
    : null;
  const nextRecipients = nextHeat === null
    ? { results: [] as NotificationHeatRecipient[] }
    : await notificationHeatRecipients(env, eventId, nextHeat.id);
  ...
  for (const recipient of rosterResult.results.filter((entry) => entry.registration_status === "ACTIVE")) {
    const resultNotification = participantNotificationStatements(env, {
      eventId,
    ...
    notificationIds.push(...resultNotification.ids);
    statements.push(...resultNotification.statements);
  }
  if (context.round === "ROUND_ONE" && finalHeatId !== null) {
    const winner = rosterResult.results.find((entry) => entry.race_entry_id === results[0].raceEntryId);
    if (winner !== undefined) {
      const finalAssignment = participantNotificationStatements(env, {
        eventId,
      ...
      notificationIds.push(...finalAssignment.ids);
      statements.push(...finalAssignment.statements);
    }
  }
  for (const recipient of nextRecipients.results) {
    ...
    statements.push(...upcoming.statements);
  }
  statements.push(env.DB.prepare(
  ...
  await publishParticipantNotifications(env, notificationIds);
...
const correctResults = async (
  ...
  const notificationIds: string[] = [];
  for (const recipient of roster.results.filter((entry) => entry.registration_status === "ACTIVE")) {
    const resultNotification = participantNotificationStatements(env, {
      eventId,
      registrationId: recipient.registration_id,
      heatId,
