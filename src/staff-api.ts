...
const pairDuck = async (
  ...
  // Both opted-in channels are durable in the same authoritative batch as the
  // assignment. Their queue messages contain only opaque outbox identifiers.
  if (assignedHeatId !== null) {
    statements.push(assignmentNotificationStatement(
      ...
      commandId,
      now,
      heat?.round === "FINAL" ? "FINAL" : "ROUND_ONE",
    ));
  }
