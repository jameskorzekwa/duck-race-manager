...
test.describe("the public Winners finale", () => {
  ...
  test("stays readable across widths, honours reduced motion, and keeps useful content without JavaScript", async ({ browser, page }) => {
    ...
    // The board is drawn in the browser, so with scripting off the page must
    // still say what this view is and how to read it rather than showing an
    // empty shell. The same element is inert while scripting is on.
    ...
    await expect(quiet.locator("[data-live-board-noscript]")).toContainText("Winners podium");
    await expect(quiet.locator("[data-live-board-noscript]")).toContainText("Turn JavaScript on");
